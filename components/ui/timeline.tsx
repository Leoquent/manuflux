    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
                <motion.span initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-red-500 font-semibold uppercase tracking-widest text-sm block mb-4">
                    Ablauf
                </motion.span>
                <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-4xl md:text-6xl font-display font-bold mt-4 mb-8 text-white max-w-4xl">
            ...
            <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
                {data.map((item, index) => (
                    ...
                    >
                        <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
                            ...
                            <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-display font-black text-white/10 select-none">
                                {item.title}
                            </h3>
                        </div>

                        <div className="relative pl-20 pr-4 md:pl-4 w-full">
                            <h3 className="md:hidden block text-2xl mb-4 text-left font-display font-black text-white/10 select-none">
                                {item.title}
                            </h3>
                            {item.content}
                        </div>
                ...
                >
                    <motion.div
                        ...
                        className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-red-700 via-red-500 to-transparent from-[0%] via-[10%] rounded-full"
                    />
                </div>
            </div>
        </div>
